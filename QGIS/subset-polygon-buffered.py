...
class BoundingBoxDivider(QgsProcessingAlgorithm):
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        overlap_meters = self.parameterAsDouble(parameters, self.OVERLAP, context)
        ...
        for input_feature in source.getFeatures():
            ...
            for row in range(num_rows):
                ...
                for col in range(num_columns):
                    # Calculate basic cell coordinates without overlap
                    ...
                    # Convert overlap from meters to degrees at this latitude
                    meters_per_degree = 111320 * math.cos(math.radians(center_y))
                    overlap_degrees = overlap_meters / meters_per_degree

                    # Apply overlap to cell boundaries
                    cell_x_min -= overlap_degrees
                    cell_x_max += overlap_degrees
                    cell_y_min -= overlap_degrees
                    cell_y_max += overlap_degrees
                    ...
                    # Create rectangle geometry with overlap
                    ...
                    attrs = {
                        ...
                        'center_lon': lon,
                        'buffer': buffer,
                        'notes': f"stub={cell_name}\nlat={lat}\nlon={lon}\nbuffer={buffer}\noverlap={round(overlap_meters, 2)}m"
                    }
